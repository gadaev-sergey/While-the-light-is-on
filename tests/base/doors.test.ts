...
import {backPoint,doorLeaf} from '../../src/base/architecture.ts';
...
test('Both approaches stop at a closed door, grip its handle, and keep eyes and lamp outside',()=>{
 for(const side of [-1,1] as const){const {w,d}=setup(side);step(w,2,-side);assert.equal(w.player.x,d.x+side*DOOR_CLEARANCE);assert.equal(w.player.moving,false);assert.equal(w.doorInteraction?.phase,'grip');assert.equal(w.doorInteraction.reach,1);
...
test('Reaching cannot be skipped by pressing both choices, and opening retains occlusion until finished',()=>{
 ...
 step(w,.8);w.action({type:'door-open'});step(w,.15,1);assert.ok(d.openness!>0&&d.openness!<1);assert.equal(w.visible({x:1200,y:539}),false);assert.equal(w.player.x,1010);
...
test('Gameplay locks permit observation, prohibit opening and survive a stale open save',()=>{
 ...
 const level=structuredClone(w.level);level.doors.find(x=>x.id===d.id)!.locked=true;const restored=new BaseWorld(level);assert.ok(restored.restore(saved));assert.equal(restored.doors.find(x=>x.id===d.id)!.open,false);assert.equal(restored.player.x,d.x+DOOR_CLEARANCE);assert.equal(restored.doorInteraction,null);assert.equal(restored.sight.peek,undefined);
});
