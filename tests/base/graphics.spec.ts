...
test('New cutouts have true alpha, and rendered light stops at closed doors and floors',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
  ...
  renderer.draw(w,0,1);const closed=luminance(1280,520),aboveClosed=luminance(1000,330);w.action({type:'interact',target:'home/kitchen-door'});for(let i=0;i<48;i++)w.update(1/60);w.action({type:'door-open'});for(let i=0;i<30;i++)w.update(1/60);renderer.draw(w,0,1);const open=luminance(1280,520),aboveOpen=luminance(1000,330);canvas.remove();return {alpha,closed,open,aboveClosed,aboveOpen};
 });
...
test('Hidden floor coverings blur while intact wall returns remain sharp',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
  ...
  const floor=[],wall=[],interior=[];for(let x=712;x<790;x+=4){floor.push(sample(x,396));interior.push(sample(x,305));}for(let y=290;y<354;y+=4)wall.push(sample(525,y));
  const result={layers,floor:contrast(floor),wall:contrast(wall),interior:contrast(interior),hidden:!w.visible({x:750,y:396})&&!w.visible({x:525,y:310})};canvas.remove();return result;
 });
 expect(result.hidden).toBe(true);expect(result.floor).toBeGreaterThan(10);expect(result.floor).toBeLessThan(150);expect(result.wall).toBeGreaterThan(200);expect(result.interior).toBeLessThan(150);expect(result.layers.indexOf('structure')).toBeGreaterThan(result.layers.indexOf('visibility'));
});
