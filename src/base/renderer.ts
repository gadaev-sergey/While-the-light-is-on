...
export const HOUSE_LAYERS=['background','walls','interactables','floors','stairs','actors','foreground','lighting','markers'] as const;
export class BaseRenderer{
 ...
 resize(){const box=this.canvas.getBoundingClientRect();this.width=Math.max(1,box.width);this.height=Math.max(1,box.height);this.dpr=Math.min(devicePixelRatio||1,2);this.canvas.width=Math.round(this.width*this.dpr);this.canvas.height=Math.round(this.height*this.dpr);this.fog.width=this.canvas.width;this.fog.height=this.canvas.height;this.wallLayer.width=this.canvas.width;this.wallLayer.height=this.canvas.height;}
 ...
 draw(w:BaseWorld,dt:number,alpha=1){
  this.level=w.level;this.layerTrace=[];const layer=(name:typeof HOUSE_LAYERS[number])=>this.layerTrace.push(name);
  ...
  layer('foreground');this.architectureEdges(c);this.stairRails(c);this.foreground(c,w);
  layer('lighting');this.darkness(w);this.transform(c);if(w.flashlight)this.flashlight(c,w);
 ...
 background(c:CanvasRenderingContext2D){
  const im=this.assets.images.district,top=Math.min(-80,this.camera.y-this.height/this.scale/2);c.drawImage(im,this.camera.x*.15-310,top,this.level.width+630,800-top);c.fillStyle='#14233145';c.fillRect(-500,top,3300,1000-top);
  ...
  for(const opening of w.openings){
   ...
   if(opening.plane==='back'){
    ...
    if(opening.state==='open'){
     const light=c.createLinearGradient(0,top,0,y-22);light.addColorStop(0,'#9ab9c210');light.addColorStop(1,'#9ab9c200');c.fillStyle=light;
     this.path(c,[{x:opening.x-opening.width*.27,y:top+opening.height*.4},{x:opening.x+opening.width*.27,y:top+opening.height*.4},{x:opening.x+opening.width*.65,y:y-22},{x:opening.x-opening.width*.1,y:y-22}]);c.fill();
    }
   }else{
 ...
 floors(c:CanvasRenderingContext2D){for(const b of this.level.buildings)for(const floor of b.floors){const y=this.floorY(floor);for(const [x,end] of floorSpans(this.level,b,floor)){
 ...
 stairRails(c:CanvasRenderingContext2D){for(const s of this.level.stairs){const y=this.floorY(s.from),y2=this.floorY(s.to);c.strokeStyle='#0c1116';c.lineWidth=4;c.beginPath();c.moveTo(s.a-10,y-39);c.lineTo(s.b+8,y2-39);c.stroke();c.strokeStyle='#86827188';c.lineWidth=1;c.stroke();for(let i=0;i<4;i++){const t=i/3,x=lerp(s.a,s.b,t),yy=lerp(y,y2,t);c.strokeStyle='#4c514a';c.lineWidth=2;c.beginPath();c.moveTo(x,yy-39);c.lineTo(x,yy+7);c.stroke();}}}
 ...
 darkness(w:BaseWorld){
  ...
  for(const room of this.level.rooms){c.fillStyle=w.explored.has(room.id)?'rgba(3,7,12,.70)':'rgba(3,6,10,.90)';c.fillRect(room.x,this.floorY(room.floor)-this.level.floorHeight+8,room.end-room.x,this.level.floorHeight+16);}
