...
export interface DaylightSource {id:string;origin:Vec;range:number;strength:number;angle:number;color:string;beamWidth:number;beamStrength:number}
...
export function daylightSources(level:CompiledLevel,doors:Door[],openings:Opening[],minutes:number):DaylightSource[]{
 ...
 for(const o of openings){if(o.state!=='open'||o.plane!=='back'||o.floor<0)continue;
  for(const sample of [-1,0,1])sources.push({id:`${o.id}:${sample}`,origin:{x:o.x+sample*o.width*.18,y:levelFloorY(level,o.floor)-o.bottom-o.height*.48+Math.abs(sample)*o.height*.06},range:o.kind==='breach'?340:280,strength:style.sun*(o.kind==='breach'?.2:.16),angle:style.angle,color:style.color,beamWidth:o.kind==='breach'?28:17,beamStrength:style.sun*(o.kind==='breach'?.6:.52)});
 }
