...
export function daylightStyle(minutes:number){
 ...
 const night=1-smooth(.01,.28,sun);
 return {sun,night,moonColor:'#9dbce8',moonAngle:2.05,exteriorDark:lerp(.73,.04,sun)-night*.035,interiorDark:.9,sky: sun<.05?'#172540':warm?'#ae8e84':'#89a5b1',skyMix:sun<.05?.22:.12+sun*.13,color,angle:lerp(.65,2.5,Math.max(0,Math.min(1,(h-6)/8)))};
}
export interface DaylightSource {id:string;origin:Vec;range:number;strength:number;angle:number;color:string;beamWidth:number;beamStrength:number;openingId?:string;apertureBeam?:boolean;celestial?:'moon'|'stars'}
...
/** Moonlight uses the same openings and occluders as sunlight. Starlight is only
 * a very weak diffuse fill at the opening, never a second set of sharp shafts. */
export function nightSources(level:CompiledLevel,doors:Door[],openings:Opening[],minutes:number):DaylightSource[]{
 const style=daylightStyle(minutes);if(style.night<=0)return [];
 const sources:DaylightSource[]=daylightSources(level,doors,openings,840).map(source=>({...source,id:`moon:${source.id}`,celestial:'moon',strength:source.strength*style.night*.26,beamStrength:source.beamStrength*style.night*.25,color:style.moonColor,angle:source.openingId?style.moonAngle:source.angle,range:source.range*.9}));
 for(const o of openings){if(o.state!=='open'||o.plane!=='back'||o.floor<0)continue;const p=openingPlacement(level,o);
  sources.push({id:`stars:${o.id}`,celestial:'stars',openingId:o.id,origin:{x:p.x,y:p.bottom-p.height*.48},range:180,strength:style.night*.016,beamWidth:0,beamStrength:0,angle:style.moonAngle,color:'#a9c8ec'});
 }return sources;
}
export const exteriorLightSources=(level:CompiledLevel,doors:Door[],openings:Opening[],minutes:number)=>[...daylightSources(level,doors,openings,minutes),...nightSources(level,doors,openings,minutes)];
