export type ImageId='district'|'materials'|'furniture'|'objects'|'dog'|'walk'|'idle'|'punch'|'damage';
export class BaseAssets{
 images={} as Record<ImageId,HTMLImageElement>;tiles:HTMLCanvasElement[]=[];damageFrames:{image:HTMLCanvasElement;mask:HTMLCanvasElement}[]=[];
 async load(progress:(n:number)=>void){
  const files:Record<ImageId,string>={district:'base/district',materials:'base/materials',furniture:'base/furniture',objects:'base/objects',dog:'base/dog',walk:'developer-walk',idle:'developer-attack',punch:'developer-left-punch',damage:'base/house-damage'};let done=0;
