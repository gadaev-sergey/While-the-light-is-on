export type ImageId='district'|'materials'|'furniture'|'objects'|'dog'|'walk'|'idle'|'punch'|'damage'|'interior'|'crate'|'doorPose';
export class BaseAssets{
 ...
 async load(progress:(n:number)=>void){
  const files:Record<ImageId,string>={district:'base/district',materials:'base/materials',furniture:'base/furniture',objects:'base/objects',dog:'base/dog',walk:'developer-walk',idle:'developer-attack',punch:'developer-left-punch',damage:'base/house-damage',interior:'base/interior-front',crate:'base/crates-front',doorPose:'base/developer-door'};let done=0;
  ...
  this.images.interior=this.keyedTexture(this.images.interior);this.images.doorPose=this.keyedTexture(this.images.doorPose);
