...
export class BaseUI{
 ...
 update(){
  ...
  this.$('#context-title').textContent=w.task?'Выполняется действие…':desc?`${desc.verb} · ${desc.title}`:'Осмотритесь';this.$('#context-detail').textContent=w.task?`${(w.task.duration-w.task.remaining).toFixed(1)} / ${w.task.duration.toFixed(1)} с · E — отменить`:desc?.detail||'Подойдите к предмету или нажмите на его значок';this.$('#context-ring').style.setProperty('--progress',w.task?`${(1-w.task.remaining/w.task.duration)*360}deg`:'0deg');
