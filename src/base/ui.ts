...
export class BaseUI{
 ...
 update(){
  ...
  const interaction=w.doorInteraction,door=w.doors.find(d=>d.id===interaction?.id);this.$('#door-panel').classList.toggle('hidden',!interaction||w.phase!=='playing');
  if(interaction&&door){
   ...
   this.$('#door-open span').textContent=door.locked?'Дверь заперта':door.open?'Закрыть дверь':'Открыть дверь';
